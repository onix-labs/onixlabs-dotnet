...
public readonly partial struct Int256
{
    ...
    public static Int256 operator <<(Int256 value, int shiftAmount)
    {
        ...
        {
            return new Int256(value.LowerBits << (shiftAmount - HalfBitWidth), UInt128.Zero);
        }

        UInt128 newUpper = (value.UpperBits << shiftAmount) | (value.LowerBits >> (HalfBitWidth - shiftAmount));
        UInt128 newLower = value.LowerBits << shiftAmount;
    ...
    public static Int256 operator >>(Int256 value, int shiftAmount)
    {
        ...
        UInt128 newUpperArith = (value.UpperBits >> shiftAmount) | (signFill << (HalfBitWidth - shiftAmount));
        UInt128 newLowerArith = (value.LowerBits >> shiftAmount) | (value.UpperBits << (HalfBitWidth - shiftAmount));
    ...
    public static Int256 operator >>>(Int256 value, int shiftAmount)
    {
        ...
        if (shiftAmount >= HalfBitWidth)
        {
            return new Int256(UInt128.Zero, value.UpperBits >> (shiftAmount - HalfBitWidth));
        }

        UInt128 newUpper = value.UpperBits >> shiftAmount;
        UInt128 newLower = (value.LowerBits >> shiftAmount) | (value.UpperBits << (HalfBitWidth - shiftAmount));
        return new Int256(newUpper, newLower);
    }
}
