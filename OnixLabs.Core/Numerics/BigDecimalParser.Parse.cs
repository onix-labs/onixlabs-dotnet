...
internal sealed partial class BigDecimalParser
{
    ...
    public BigDecimal Parse(ReadOnlySpan<char> value, NumberStyles style)
    {
        ...
        if (style == NumberStyles.None) style = DetectNumberStyle(value);

        if (style == NumberStyles.Float) return ParseFloat(value);
        if (style == NumberStyles.Integer) return ParseInteger(value);
        if (style == NumberStyles.HexNumber) return ParseHexadecimal(value);
        throw new FormatException("Input value is not in a valid format.");
    }
}
