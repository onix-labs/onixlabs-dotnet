...
public readonly partial struct Pressure<T>
{
    ...
    /// <summary>
    /// Formats the value using <c>&lt;force&gt;/&lt;area&gt;[:scale]</c> (e.g. <c>kg*m/s²/sqm:3</c> for pascals).
    /// </summary>
    public string ToString(ReadOnlySpan<char> format, IFormatProvider? formatProvider = null)
    {
        ...
        if (forceSpecifier.IsEmpty)
            throw new FormatException("Pressure format force specifier must not be empty.");

        if (areaSpecifier.IsEmpty)
            throw new FormatException("Pressure format area specifier must not be empty.");

        T value = Left.ValueOf(forceSpecifier) / Right.ValueOf(areaSpecifier);
        T rounded = scale > 0 ? T.Round(value, scale) : value;

        return $"{rounded.ToString($"N{scale}", formatProvider ?? CultureInfo.CurrentCulture)} "
               + $"{new string(forceSpecifier)}/{new string(areaSpecifier)}";
    }
}
