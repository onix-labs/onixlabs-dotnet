...
{
    /// <summary>
    /// The default <see cref="NumberStyles"/> used when parsing a <see cref="UInt256"/> value without an explicit style argument.
    /// </summary>
    private const NumberStyles DefaultNumberStyles = NumberStyles.Integer;

    /// <summary>Parses the specified string into a <see cref="UInt256"/> value.</summary>
    ...
    /// <exception cref="FormatException">Thrown when <paramref name="value"/> is not in a correct format.</exception>
    /// <exception cref="OverflowException">Thrown when <paramref name="value"/> exceeds the range of <see cref="UInt256"/>.</exception>
    public static UInt256 Parse(ReadOnlySpan<char> value, NumberStyles style, IFormatProvider? provider = null)
    {
        BigInteger big = BigInteger.Parse(value, style, provider);
        if (big.Sign < 0) throw new OverflowException($"Value was either too large or too small for the specified type: {nameof(UInt256)}.");
        if (big.GetBitLength() > BitWidth) throw new OverflowException($"Value was either too large or too small for the specified type: {nameof(UInt256)}.");
        return (UInt256)big;
    }

    /// <summary>Tries to parse the specified string into a <see cref="UInt256"/> value.</summary>
    /// <param name="value">The value to parse.</param>
    ...
    /// <param name="result">When this method returns, contains the parsed value if successful; otherwise, the default value.</param>
    /// <returns>Returns <see langword="true"/> if the value was parsed successfully; otherwise, <see langword="false"/>.</returns>
    public static bool TryParse(string? value, IFormatProvider? provider, out UInt256 result) => TryParse(value.AsSpan(), provider, out result);
    ...
    /// <summary>Tries to parse the specified span into a <see cref="UInt256"/> value.</summary>
    /// <param name="value">The value to parse.</param>
    /// <param name="result">When this method returns, contains the parsed value if successful; otherwise, the default value.</param>
    /// <returns>Returns <see langword="true"/> if the value was parsed successfully; otherwise, <see langword="false"/>.</returns>
    public static bool TryParse(ReadOnlySpan<char> value, out UInt256 result) => TryParse(value, CultureInfo.CurrentCulture, out result);
    ...
    /// <param name="result">When this method returns, contains the parsed value if successful; otherwise, the default value.</param>
    /// <returns>Returns <see langword="true"/> if the value was parsed successfully; otherwise, <see langword="false"/>.</returns>
    public static bool TryParse(ReadOnlySpan<char> value, IFormatProvider? provider, out UInt256 result) => TryParse(value, DefaultNumberStyles, provider, out result);
