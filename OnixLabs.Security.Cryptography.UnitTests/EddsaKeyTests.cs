...
public sealed class EddsaKeyTests
{
    ...
    [Theory(DisplayName = "EdDSA must satisfy Project Wycheproof Ed25519 test vectors")]
    [WycheproofEd25519Data]
    public void EddsaMustSatisfyWycheproofEd25519TestVectors(int tcId, byte[] publicKeyBytes, byte[] message, byte[] signature, bool expected)
    {
        // Given
        _ = tcId;
        EddsaPublicKey publicKey = new(publicKeyBytes);

        // When
        bool actual = publicKey.IsDataValid(signature, message);

        // Then
        Assert.Equal(expected, actual);
    }

    [Fact(DisplayName = "EdDSA verification should reject signatures with non-canonical S (S >= L)")]
    public void EddsaVerificationShouldRejectSignaturesWithNonCanonicalS()
    {
        // Given
        EddsaPrivateKey privateKey = EddsaPrivateKey.Create();
        EddsaPublicKey publicKey = privateKey.GetPublicKey();
        ReadOnlySpan<byte> data = Salt.CreateNonZero(64).AsReadOnlySpan();
        byte[] signature = privateKey.SignData(data);

        // When
        // Overwrite the S component (signature[32..64]) with L itself. L is the curve order;
        // its 32-byte little-endian encoding is the boundary at which scalars become non-canonical.
        ReadOnlySpan<byte> littleEndianL =
        [
        ...
        littleEndianL.CopyTo(signature.AsSpan(32));

        // Then
        Assert.False(publicKey.IsDataValid(signature, data));
    }

    [Fact(DisplayName = "EdDSA verification should reject public keys with non-canonical y (y >= p)")]
    public void EddsaVerificationShouldRejectNonCanonicalY()
    {
        // Given
        // y = p = 2^255 - 19, little-endian encoding [0xed, 0xff*30, 0x7f]. Top bit (sign) is zero.
        // Any value in [p, 2^255) is non-canonical and TryDecode must reject it.
        byte[] nonCanonicalPublicKey =
        [
        ...
        EddsaPublicKey publicKey = new(nonCanonicalPublicKey);
        ReadOnlySpan<byte> data = Salt.CreateNonZero(32).AsReadOnlySpan();
    ...
    [Fact(DisplayName = "EdDSA verification should fail when S is bit-flipped (malleability)")]
    public void EddsaVerificationShouldFailWhenSIsBitFlipped()
    {
        // Given
        EddsaPrivateKey privateKey = EddsaPrivateKey.Create();
        EddsaPublicKey publicKey = privateKey.GetPublicKey();
        ...
        // When — flip a low-order bit in the S half of the signature.
        signature[40] ^= 0x01;
    ...
    [Fact(DisplayName = "EdDSA verification should fail when R is bit-flipped (malleability)")]
    public void EddsaVerificationShouldFailWhenRIsBitFlipped()
    {
        // Given
        EddsaPrivateKey privateKey = EddsaPrivateKey.Create();
        EddsaPublicKey publicKey = privateKey.GetPublicKey();
        ...
        // When — flip a low-order bit in the R half of the signature.
        signature[5] ^= 0x01;
    ...
    [Fact(DisplayName = "EdDSA cofactored verification accepts signatures with a low-order public key")]
    public void EddsaCofactoredVerificationAcceptsLowOrderPublicKey()
    {
        // Given
        // The all-zero encoding decodes to the point of order 4 (y = 0, x = 0 sign bit 0).
        // RFC 8032 §5.1.7 does not mandate rejection; cofactored verification (the implementation here)
        // multiplies both sides by 8, so [8][k]A = 0 for any k. Both sides therefore reduce to
        // 8·[S]B vs 8·R, and a signature constructed with R = [s]B, S = s for any s ∈ [0, L) verifies.
        // We construct such a signature explicitly and assert that verification accepts it. This
        // documents the cofactored behaviour; switching to strict (non-cofactored) verification
        // would reject the same input.
        byte[] lowOrderPublicKey = new byte[32]; // y = 0, sign bit 0 — a point of order 4.
        EddsaPublicKey publicKey = new(lowOrderPublicKey);

        // Build a signature whose R = [s]B and S = s for s = 1. Then 8·[S]B = 8·R, and 8·[k]A = 0,
        // so the cofactored check holds regardless of message or hash output k.
        byte[] signature = new byte[64];
        // R = [1]B — encoding of the Ed25519 base point B per RFC 8032 §5.1.
        ReadOnlySpan<byte> encodedBasePoint =
        [
        ...
        encodedBasePoint.CopyTo(signature.AsSpan(0, 32));
        signature[32] = 0x01; // S = 1, little-endian.

        // When / Then — cofactored verification accepts; the test documents the behaviour.
