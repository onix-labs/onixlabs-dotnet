...
internal sealed partial class BigDecimalParser
{
    ...
    public BigDecimal Parse(ReadOnlySpan<char> value, NumberStyles style)
    {
        ...
        return (style == NumberStyles.None ? DetectNumberStyle(sanitized) : style) switch
        {
            NumberStyles.Float => ParseFloat(sanitized),
            NumberStyles.Integer => ParseInteger(sanitized),
            NumberStyles.HexNumber => ParseHexadecimal(sanitized),
