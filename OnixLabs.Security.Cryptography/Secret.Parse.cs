...
{
    /// <inheritdoc/>
    public static Secret Parse(string value, IFormatProvider? provider = null) =>
        Parse(value.AsSpan(), provider);

    /// <inheritdoc/>
    public static Secret Parse(ReadOnlySpan<char> value, IFormatProvider? provider = null) =>
        Secret.ParseOrThrow(value, provider);

    /// <inheritdoc/>
    public static bool TryParse(string? value, IFormatProvider? provider, out Secret result) =>
