...
{
    /// <summary>
    /// Compares two <see cref="UInt256"/> values and returns an integer that indicates their relative order.
    /// </summary>
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="obj"/> is not <see langword="null"/> and not a <see cref="UInt256"/>.</exception>
    ...
    /// <summary>
    /// Compares the current instance with another <see cref="UInt256"/> value.
    /// </summary>
    /// <param name="other">A <see cref="UInt256"/> value to compare with this instance.</param>
    /// <returns>Returns a negative integer, zero, or a positive integer indicating the relative order.</returns>
    ...
    public static bool operator <(UInt256 left, UInt256 right) => Compare(left, right) < 0;
