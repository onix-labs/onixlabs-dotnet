...
public readonly partial struct Density<T>
{
    /// <inheritdoc/>
    /// <remarks>
    /// The denominator is set to <c>1 m³</c> rather than <see cref="Volume{T}.Zero"/> to avoid a <c>0 / 0</c> magnitude,
    ...
    /// </remarks>
    public static Density<T> Zero => new(Mass<T>.Zero, Volume<T>.FromCubicMeters(T.One));

    private const string DefaultFormat = "kg/cum";
}
