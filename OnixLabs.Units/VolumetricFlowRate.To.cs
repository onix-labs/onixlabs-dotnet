...
public readonly partial struct VolumetricFlowRate<T>
{
    ...
    /// </summary>
    /// <param name="format">The format to use. Composite specifiers take the form <c>&lt;volume&gt;/&lt;time&gt;[:scale]</c> (for example <c>cum/s</c> or <c>L/s:0</c>).</param>
    ...
    public string ToString(ReadOnlySpan<char> format, IFormatProvider? formatProvider = null)
    {
        ...
        if (slashIndex < 0)
            throw new FormatException($"VolumetricFlowRate format must contain a '/' separator between the volume and time specifiers (e.g. '{DefaultFormat}').");

        ReadOnlySpan<char> volumeSpecifier = format[..slashIndex];
        ...
        else
        {
            ...
            if (!int.TryParse(rest[(colonIndex + 1)..], NumberStyles.Integer, CultureInfo.InvariantCulture, out scale))
                throw new FormatException("VolumetricFlowRate format scale must contain only decimal digits.");
        }

        if (volumeSpecifier.IsEmpty)
            throw new FormatException("VolumetricFlowRate format volume specifier must not be empty.");

        if (timeSpecifier.IsEmpty)
            throw new FormatException("VolumetricFlowRate format time specifier must not be empty.");

        T value = Left.ValueOf(volumeSpecifier) / Right.ValueOf(timeSpecifier);
        T rounded = scale > 0 ? T.Round(value, scale) : value;

        return $"{rounded.ToString($"N{scale}", formatProvider ?? CultureInfo.CurrentCulture)} "
               + $"{new string(volumeSpecifier)}/{new string(timeSpecifier)}";
    }
}
