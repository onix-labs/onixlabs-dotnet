...
public readonly partial struct Hmac
{
    ...
    public static async Task<Hmac> ComputeSha2Hmac512Async(string value, string key)
    {
        return await ComputeSha2Hmac512Async(value, key, Encoding.Default);
    }
    ...
    /// <returns>Returns a <see cref="Hmac"/> of the input value and key.</returns>
    public static async Task<Hmac> ComputeSha2Hmac512Async(string value, string key, Encoding encoding)
    {
        byte[] valueBytes = encoding.GetBytes(value);
        byte[] keyBytes = encoding.GetBytes(key);
        return await ComputeSha2Hmac512Async(valueBytes, keyBytes);
    }
    ...
    /// <param name="key">The key for which to compute a HMAC.</param>
    /// <returns>Returns a <see cref="Hmac"/> of the input value and key.</returns>
    public static async Task<Hmac> ComputeSha2Hmac512Async(byte[] value, byte[] key)
    {
        return await ComputeHmacAsync(value, key, HashAlgorithmType.Sha2Hmac512);
    }
}
