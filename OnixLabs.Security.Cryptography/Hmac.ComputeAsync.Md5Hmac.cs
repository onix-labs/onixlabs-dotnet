...
namespace OnixLabs.Security.Cryptography
{
    public readonly partial struct Hmac
    {
        ...
        public static async Task<Hmac> ComputeMd5HmacAsync(string value, string key)
        {
            return await ComputeMd5HmacAsync(value, key, Encoding.Default);
        }
        ...
        /// <returns>Returns a <see cref="Hmac"/> of the input value and key.</returns>
        public static async Task<Hmac> ComputeMd5HmacAsync(string value, string key, Encoding encoding)
        {
            byte[] valueBytes = encoding.GetBytes(value);
            byte[] keyBytes = encoding.GetBytes(key);

            return await ComputeMd5HmacAsync(valueBytes, keyBytes);
        }
        ...
        /// <param name="key">The key for which to compute a HMAC.</param>
        /// <returns>Returns a <see cref="Hmac"/> of the input value and key.</returns>
        public static async Task<Hmac> ComputeMd5HmacAsync(byte[] value, byte[] key)
        {
            return await ComputeHmacAsync(value, key, HashAlgorithmType.Md5Hmac);
        }
    }
}
