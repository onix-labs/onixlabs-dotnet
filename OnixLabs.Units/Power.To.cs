...
public readonly partial struct Power<T>
{
    ...
    /// <summary>
    /// Formats the value using <c>&lt;energy&gt;/&lt;time&gt;[:scale]</c> (e.g. <c>kg*m/s²*m/s:3</c> for watts).
    /// </summary>
    public string ToString(ReadOnlySpan<char> format, IFormatProvider? formatProvider = null)
    {
        ...
        else
        {
            ...
            if (!int.TryParse(format[(colonIndex + 1)..], NumberStyles.Integer, CultureInfo.InvariantCulture, out scale))
                throw new FormatException("Power format scale must contain only decimal digits.");
        }

        // Named-unit alias (W, kW, mW, MW, GW, ...).
        ...
        // Energy-spec contains '/' (via its inner acceleration), time-spec does not. Split on last '/'.
        int lastSlash = unitPart.LastIndexOf('/');
        if (lastSlash < 0)
            throw new FormatException($"Power format must contain a '/' separator between energy and time (e.g. 'kg*m/s²*m/s').");
        ...
        if (energySpecifier.IsEmpty)
            throw new FormatException("Power format energy specifier must not be empty.");

        if (timeSpecifier.IsEmpty)
            throw new FormatException("Power format time specifier must not be empty.");

        T value = Left.ValueOf(energySpecifier) / Right.ValueOf(timeSpecifier);
        T rounded = scale > 0 ? T.Round(value, scale) : value;

        return $"{rounded.ToString($"N{scale}", formatProvider ?? CultureInfo.CurrentCulture)} "
               + $"{new string(energySpecifier)}/{new string(timeSpecifier)}";
    }
}
