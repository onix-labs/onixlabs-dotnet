...
public readonly partial struct Torque<T>
{
    ...
    /// </summary>
    /// <param name="format">
    /// The format to use. Composite specifiers take the form <c>&lt;force&gt;*&lt;distance&gt;[:scale]</c> where
    /// <c>&lt;force&gt;</c> is itself the composite shape <c>&lt;mass&gt;*&lt;acceleration&gt;</c>. Examples:
    /// <c>kg*m/s²*m:3</c> (Newton-metres), <c>g*cm/s²*cm:3</c> (dyne-centimetres).
    /// </param>
    ...
    public string ToString(ReadOnlySpan<char> format, IFormatProvider? formatProvider = null)
    {
        ...
        else
        {
            ...
            if (!int.TryParse(format[(colonIndex + 1)..], NumberStyles.Integer, CultureInfo.InvariantCulture, out scale))
                throw new FormatException("Torque format scale must contain only decimal digits.");
        }
        ...
        if (lastStar < 0)
            throw new FormatException($"Torque format must contain a '*' separator between the force and distance specifiers (e.g. '{DefaultFormat}').");
        ...
        if (forceSpecifier.IsEmpty)
            throw new FormatException("Torque format force specifier must not be empty.");

        if (distanceSpecifier.IsEmpty)
            throw new FormatException("Torque format distance specifier must not be empty.");

        T value = Left.ValueOf(forceSpecifier) * Right.ValueOf(distanceSpecifier);
        T rounded = scale > 0 ? T.Round(value, scale) : value;

        return $"{rounded.ToString($"N{scale}", formatProvider ?? CultureInfo.CurrentCulture)} "
               + $"{new string(forceSpecifier)}*{new string(distanceSpecifier)}";
    }
}
