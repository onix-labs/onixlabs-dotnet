...
{
    /// <inheritdoc/>
    public static EcdhPublicKey Parse(string value, IFormatProvider? provider = null) =>
        Parse(value.AsSpan(), provider);

    /// <inheritdoc/>
    public static EcdhPublicKey Parse(ReadOnlySpan<char> value, IFormatProvider? provider = null) =>
        EcdhPublicKey.ParseOrThrow(value, provider);

    /// <inheritdoc/>
    public static bool TryParse(string? value, IFormatProvider? provider, out EcdhPublicKey result) =>
