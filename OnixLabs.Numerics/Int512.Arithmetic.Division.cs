...
{
    /// <summary>Computes the quotient of two <see cref="Int512"/> values.</summary>
    ...
    public static Int512 operator /(Int512 left, Int512 right)
    {
        ...
        if (left == MinValue && right == NegativeOne) throw new OverflowException($"Value was either too large or too small for the specified type: {nameof(Int512)}.");
        ...
        UInt512 quotient = absLeft / absRight;
        Int512 signed = ReinterpretAsSigned(quotient);
        return resultNegative ? -signed : signed;
    }

    /// <summary>Computes the quotient and remainder of two <see cref="Int512"/> values.</summary>
    /// <param name="left">The dividend.</param>
    /// <param name="right">The divisor.</param>
    ...
    public static (Int512 Quotient, Int512 Remainder) DivRem(Int512 left, Int512 right)
    {
        ...
        UInt512 absLeft = AbsToUnsigned(left);
        UInt512 absRight = AbsToUnsigned(right);
        (UInt512 absQuotient, UInt512 absRemainder) = UInt512.DivRem(absLeft, absRight);

        Int512 quotient = ReinterpretAsSigned(absQuotient);
        Int512 remainder = ReinterpretAsSigned(absRemainder);
