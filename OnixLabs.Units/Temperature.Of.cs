...
public readonly partial struct Temperature<T>
{
    ...
    public T ValueOf(ReadOnlySpan<char> specifier) => new string(specifier).ToUpperInvariant() switch
    {
        CelsiusSpecifier => Celsius,
        DelisleSpecifier => Delisle,
        FahrenheitSpecifier => Fahrenheit,
        KelvinSpecifier => Kelvin,
        NewtonSpecifier => Newton,
        RankineSpecifier => Rankine,
        ReaumurSpecifier => Reaumur,
        RomerSpecifier => Romer,
    ...
    public string SymbolOf(ReadOnlySpan<char> specifier) => new string(specifier).ToUpperInvariant() switch
    {
        CelsiusSpecifier => CelsiusSymbol,
        DelisleSpecifier => DelisleSymbol,
        FahrenheitSpecifier => FahrenheitSymbol,
        KelvinSpecifier => KelvinSymbol,
        NewtonSpecifier => NewtonSymbol,
        RankineSpecifier => RankineSymbol,
        ReaumurSpecifier => ReaumurSymbol,
        RomerSpecifier => RomerSymbol,
        _ => throw ArgumentException.InvalidFormat(specifier, ValidSpecifiers)
    };
}
