...
public readonly partial struct Energy<T>
{
    ...
    // `Distance.ValueOf("m") = 1`, giving the human-readable "sum kg*m/s²*m" rendering (i.e. value in joules).

    /// <inheritdoc/>
    public static Energy<T> Add(Energy<T> left, Energy<T> right)
    {
        T sum = left.Magnitude + right.Magnitude;
        return new Energy<T>(ForceInNewtons(sum), Distance<T>.FromMeters(T.One));
    }

    /// <inheritdoc/>
    public static Energy<T> Subtract(Energy<T> left, Energy<T> right)
    {
        T difference = left.Magnitude - right.Magnitude;
        return new Energy<T>(ForceInNewtons(difference), Distance<T>.FromMeters(T.One));
    }
