...
public readonly partial struct ElectricCharge<T>
{
    ...
    /// <summary>Formats using <c>&lt;current&gt;*&lt;time&gt;[:scale]</c> (e.g. <c>A*s:3</c> for coulombs).</summary>
    public string ToString(ReadOnlySpan<char> format, IFormatProvider? formatProvider = null)
    {
        ...
        if (currentSpecifier.IsEmpty || timeSpecifier.IsEmpty)
            throw new FormatException("ElectricCharge format current and time specifiers must be non-empty.");

        T value = Left.ValueOf(currentSpecifier) * Right.ValueOf(timeSpecifier);
        T rounded = scale > 0 ? T.Round(value, scale) : value;

        return $"{rounded.ToString($"N{scale}", formatProvider ?? CultureInfo.CurrentCulture)} "
               + $"{new string(currentSpecifier)}*{new string(timeSpecifier)}";
    }
}
