...
{
    /// <inheritdoc/>
    public static EcdhPrivateKey Parse(string value, IFormatProvider? provider = null) =>
        Parse(value.AsSpan(), provider);

    /// <inheritdoc/>
    public static EcdhPrivateKey Parse(ReadOnlySpan<char> value, IFormatProvider? provider = null) =>
        EcdhPrivateKey.ParseOrThrow(value, provider);

    /// <inheritdoc/>
    public static bool TryParse(string? value, IFormatProvider? provider, out EcdhPrivateKey result) =>
