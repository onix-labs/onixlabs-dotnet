...
public readonly partial struct Energy<T>
{
    ...
    /// <c>kg*m/s²*m:3</c> (joules), <c>g*cm/s²*cm:3</c> (ergs).
    /// </param>
    ...
    public string ToString(ReadOnlySpan<char> format, IFormatProvider? formatProvider = null)
    {
        ...
        if (forceSpecifier.IsEmpty)
            throw new FormatException("Energy format force specifier must not be empty.");

        if (distanceSpecifier.IsEmpty)
            throw new FormatException("Energy format distance specifier must not be empty.");
